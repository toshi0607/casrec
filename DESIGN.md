...
- 録画画面のSettingsセクションには「録画時間の上限」(なし / 30分 / 1時間 / 2時間 / 3時間)を置く。上限は手動開始・メニューバー・⌥⌘R・予約のすべての開始経路に適用され、予約は予約時点の上限を保持する。予約セクションでは開始日時を指定し、予約時点の対象・設定(領域指定を含む)を保持して、発火時は最新のソース一覧から対象を再解決して開始する。
- 予約中は開始時刻と分単位の残り時間、キャンセルを表示する。予約は1件のみで、アプリ終了時には消える。
- メニューバーには録画状態を示すステータスアイテムを常駐させ、開始/停止、メインウィンドウを開く、終了を提供する。Dockアイコンとメインウィンドウは維持する。
- 録画失敗は録画タブ内の failed バナーに一本化する。ライブラリタブ表示中はバナーが見えないため、メニューバーの failed 専用アイコンを一次シグナルとする。
...
| ライブラリタブ表示中の録画失敗通知 | 解消 (2026-08-04) | 録画エラーアラートを廃止し、録画タブ内の failed バナーへ一本化。ライブラリタブ滞在中はメニューバーの failed 専用アイコンを一次シグナルとする (§7) |
