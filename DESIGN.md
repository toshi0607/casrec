...
- ffmpegへ渡す録画入力はMOV demuxerと`file`プロトコルへ固定し、外部data referenceと絶対aliasを無効化する。標準入力を読ませず、stderrは末尾16 KiBだけを保持する。プロセス期限は入力時間の2倍を基本に30分〜12時間へ制限し、キャンセル時はTERM後にKILLする。GIF/修復は同一ディレクトリのUUID一時出力へ書き、非上書きrenameの成功時だけ最終名へ公開する。通常終了時はアプリ所有のジョブキューをキャンセルし、子プロセス終了を確認してからアプリを終了する。
