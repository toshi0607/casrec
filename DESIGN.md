...
破損救済(Phase 3): 起動時に書き込み中断らしき .mov(finalize完了マーカーの無いファイル)を検出し、ffmpeg があれば remux(`-c copy`)で `-recovered.mov` を生成する。**原本は破壊しない**。
...
- 行アクション: QuickLookプレビュー / Finderで表示 / **圧縮**(プリセット: 高圧縮HEVC・互換H.264) / **GIF変換**(ffmpeg検出時のみ活性) / 削除(ゴミ箱へ)。
...
- 圧縮(AVAssetExportSession、高圧縮HEVC / 互換H.264プリセット)、GIF変換(ffmpeg検出)
- 破損ファイルの検出と修復導線(§5.5)
- 保存先変更UI(§7)
