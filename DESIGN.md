...
| 署名 | `CasRec Release` の安定した自己署名identityでTCC許可を維持し、Hardened Runtime・マイクentitlement・timestampを付与する。`make verify-bundle` とCIは署名およびruntimeフラグを検証し、releaseはさらに期待するleaf hashを検証する |
| プロジェクト | SwiftPMパッケージ(executable)+ Makefileで.appバンドル生成(開発機にXcodeが無いため。導入時はPackage.swiftをXcodeで直接開ける)。ディレクトリは§10 |

### セキュリティ境界

- 予約・メニューバー・ホットキーからウィンドウを再解決する際は、`windowID`・所有プロセスID・bundle IDが一致する候補がちょうど1件の場合だけ開始する。タイトルや表示上のアプリ名は認可属性に使わず、対象消滅・所有者変更・曖昧性は再選択を要求してfail closedとする。
- ライブラリ走査は全エントリを保持しつつ、メタデータ取得を最大4並列に制限する。GIFの時間計算は10,000フレームまでとし、超過ファイルは時間不明のまま一覧へ残す。
- ffmpegへ渡す録画入力はMOV demuxerと`file`プロトコルへ固定し、外部data referenceと絶対aliasを無効化する。標準入力を読ませず、stderrは末尾16 KiBだけを保持する。プロセス期限は入力時間の2倍を基本に30分〜12時間へ制限し、キャンセル時はTERM後にKILLする。圧縮/GIF/修復は同一ディレクトリのUUID一時出力へ書き、非上書きrenameの成功時だけ最終名へ公開する。通常終了時はアプリ所有のジョブキューをキャンセルし、子プロセス終了を確認してからアプリを終了する。
...
### Phase 4 — 磨き(完結)
- ~~録画予約タイマー(配信開始時刻に自動開始)~~ **実装済み**: 予約時点の対象・設定・共通の録画時間上限を保持し、発火時に最新ソースからowner-bound identityが一意に完全一致する場合だけ再解決する。対象が消滅・変更・曖昧になった場合は開始しない。上限は手動開始・メニューバー・⌥⌘R・予約の全経路に適用する。1件予約、idle状態ガード、待機中スリープ抑止を提供する。予約はアプリ起動中のみ有効。
