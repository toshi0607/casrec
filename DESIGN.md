...
- 録画画面には開始日時と任意の録画時間上限を指定する予約セクションを置く。予約時点の対象・設定(領域指定を含む)を保持し、発火時は最新のソース一覧から対象を再解決して開始する。
...
### Phase 4 — 磨き(完結)
- ~~録画予約タイマー(配信開始時刻に自動開始)~~ **実装済み**: 予約時点の対象・設定を保持し、発火時に最新ソースをID→同一アプリ名+タイトル→同一アプリ名の順で再解決する。1件予約、任意の録画時間上限、idle状態ガード、待機中スリープ抑止を提供する。予約はアプリ起動中のみ有効。
