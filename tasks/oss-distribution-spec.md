...
| **H2** | `CasRec Release` の秘密鍵を `.p12` で書き出し、リポジトリ外の安全な場所（パスワードマネージャ等）へバックアップする | **未了 — toshi0607 が実施**。§3.2 参照 |
| **H3** | 署名した `.app` で、画面収録とマイク録音が実際に動くことを確認する（Hardened Runtime 有効化の実証。GUI と TCC が必要なため自動化不可） | 未了。C1 完了後に実施 |
...
中間生成物（`key.pem` / `tmp.p12`）は検証後に削除済み。秘密鍵はログインキーチェーンにのみ存在する。**したがって H2 のバックアップが未了の間、この鍵は単一障害点である。**
