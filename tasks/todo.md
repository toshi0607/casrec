...
- [x] 静的レビュー: 仕様10項目対応・クロック注入の決定的テスト(仮想時計)・Recording/Capture層無変更。**Medium指摘1件**: 上限自動停止が「予約とは別の手動録画」を止めうる誤爆エッジ(発火時idleガードは開始後の手動介入をカバーしない)→ PRコメントで修正案提示(stop直前にstartedAt照合)、追いコミット待ち。Low2件: ウィンドウ閉中発火でsleep activity残留(再オープンで解消)/ 過去時刻予約の即発火ガードなし
...
- 実機未消化(テスト担保): 自動停止(最小プリセット30分のため)、フォールバック再解決3経路、idle以外スキップ、スリープ明け発火(§11 UNVERIFIED行として管理)
